(()=>{
 const AI_KEY='businessai-ai-history';
 ...
 const statusText=()=>lang()==='uk'?'Повну копію з історією AI підготовлено':lang()==='en'?'Full backup with AI history prepared':'Úplná záloha s AI históriou je pripravená';
 ...
 document.addEventListener('change',event=>{
  ...
  const reader=new FileReader();reader.onload=()=>{try{const payload=JSON.parse(String(reader.result||''));if(Array.isArray(payload?.aiHistory)){localStorage.setItem(AI_KEY,JSON.stringify(payload.aiHistory));window.dispatchEvent(new CustomEvent('businessai:ai-history-changed'))}}catch(_){}};reader.readAsText(file);
 },true);
})();