(()=>{
 const KEY='businessai-workspace-tab-v2';
 const tabs=[
 ...
 let active=localStorage.getItem(KEY)||'ai';
 if(!tabs.some(x=>x.key===active))active='ai';
 const style=document.createElement('style');
 style.textContent=`
  body.businessai-tabs-ready{overflow-x:hidden}
  .businessai-workspace-tabs{position:sticky;top:0;z-index:40;background:rgba(5,15,27,.97);backdrop-filter:blur(14px);border-bottom:1px solid var(--line);box-shadow:0 12px 28px rgba(0,0,0,.18)}
  .businessai-workspace-tabs-inner{max-width:1180px;margin:0 auto;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:7px;padding:9px 18px;overflow:visible}
  .businessai-workspace-tab{min-width:0;min-height:40px;border:1px solid var(--line);background:#0b1a2c;color:var(--muted);border-radius:12px;padding:7px 8px;font:inherit;font-size:.86rem;font-weight:700;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .businessai-workspace-tab:hover{color:var(--text);border-color:rgba(255,255,255,.22)}
  .businessai-workspace-tab.active{background:var(--accent);color:#07111f;border-color:var(--accent);box-shadow:0 0 0 1px rgba(84,226,178,.12)}
  .businessai-workspace-tab span{margin-right:4px}
  ...
  body.businessai-tabs-ready #assistant{padding-top:24px}
  body.businessai-tabs-ready #business-dashboard,body.businessai-tabs-ready #idea-validator,body.businessai-tabs-ready #market-analysis,body.businessai-tabs-ready #calculator,body.businessai-tabs-ready #business-plan-builder,body.businessai-tabs-ready #marketing-builder,body.businessai-tabs-ready #documents-builder,body.businessai-tabs-ready #sales-pipeline,body.businessai-tabs-ready #cashflow-tracker,body.businessai-tabs-ready #goals-tracker,body.businessai-tabs-ready #business-health,body.businessai-tabs-ready #weekly-review{padding-top:30px}
  @media(max-width:980px){.businessai-workspace-tabs-inner{grid-template-columns:repeat(5,minmax(0,1fr));padding:8px 14px}.businessai-workspace-tab{font-size:.82rem;padding:7px 6px}}
  @media(max-width:680px){.businessai-workspace-tabs-inner{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;padding:8px 10px}.businessai-workspace-tab{min-height:38px;font-size:.78rem;padding:6px 5px}.businessai-workspace-tab span{margin-right:3px}body.businessai-tabs-ready #assistant{padding-top:18px}}
  @media(max-width:390px){.businessai-workspace-tabs-inner{grid-template-columns:repeat(2,minmax(0,1fr))}}
 `;
 ...
 function renderTabs(){
  inner.innerHTML=tabs.map(item=>`<button type="button" class="businessai-workspace-tab${item.key===active?' active':''}" data-workspace-tab="${item.key}" role="tab" aria-selected="${item.key===active?'true':'false'}" title="${tabLabel(item)}"><span>${item.icon}</span>${tabLabel(item)}</button>`).join('');
 ...
 function applyVisibility(){
  ...
  const workspace=document.querySelector('#business-workspace');
  if(workspace)workspace.classList.toggle('businessai-tab-hidden',active!=='overview');
 ...
 function activate(key,scroll){
  if(!tabs.some(x=>x.key===key))key='ai';
